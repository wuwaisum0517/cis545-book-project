...
def book_recommendation():
    ...
    def execute_clean_data_sql_query(mydb, query, column_names,debug_mode):
        ...
        return result_df
    def get_rating(book,mydb, debug_mode):
        """
        UNUSED FUNCTION, for filtering rows
        explicit_df = all_ratings[all_ratings['Book-Rating'] != 0]
        """
        query = "SELECT Book_Title, COUNT(*) as count, AVG(Book_Rating) as mean FROM all_rating WHERE Book_Rating != 0 AND Book_Title = \"{}\"".format(book) + " GROUP BY Book_Title"
        column_names = ['book_title', 'count', 'mean']
    ...
    def visualizing_recommendation_data (recommendation_list):
        ...
        output: code for showing result properly (Also pull rating data from database)
        """
        for recommendation in recommendation_list:

            #first item
            title, author, image_url, similarity_score = recommendation[0], recommendation[1], recommendation[2], recommendation[3]
            rating_of_book = get_rating(title, mydb, debug_mode)
            st.subheader(title)
            ...
            with col3:
                ...
                st.write("Total Ratings:"+str("{}".format(rating_of_book['count'][0])))
                st.write("Average Rating:"+str("{:.2f}".format(rating_of_book['mean'][0])))
            st.markdown("""---""")
        return
    ...
    try:
        ...
        default_book_list = ['2nd Chance', '1984',"Before Women Had Wings (Ballantine Reader's Circle)"]
        ...
        # multiselect bar
        st.subheader("This system gives book recommendations based on the similarity of the book you choose!")
        book_input = st.multiselect(
            "Choose at most 3 books to see our recommendation! ", list(df_title_only), default_book_list
        )
        st.markdown("""---""")
